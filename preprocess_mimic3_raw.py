...
from utils import reformat, write_discharge_summaries_raw
from dataloader import build_vocab, concat_data, split_data_raw
...
min_sentence_len = 3
disch_full_file = write_discharge_summaries_raw("%s/disch_full_raw.csv" % args.MIMIC_3_DIR, min_sentence_len, '%s/NOTEEVENTS.csv' % (args.MIMIC_3_DIR), raw=True)


df = pd.read_csv('%s/disch_full_raw.csv' % args.MIMIC_3_DIR)
...
sorted_file = '%s/disch_full_raw.csv' % args.MIMIC_3_DIR
df.to_csv(sorted_file, index=False)

# step 4: link notes with their code
labeled = concat_data('%s/ALL_CODES_filtered.csv' % args.MIMIC_3_DIR, sorted_file, '%s/notes_labeled_raw.csv' % args.MIMIC_3_DIR)
...
fname = '%s/notes_labeled_raw.csv' % args.MIMIC_3_DIR
base_name = "%s/disch_raw" % args.MIMIC_3_DIR #for output
tr, dv, te = split_data_raw(fname, base_name, args.MIMIC_3_DIR)
...
for splt in ['train', 'dev', 'test']:
    filename = '%s/disch_raw_%s_split.csv' % (args.MIMIC_3_DIR, splt)
    df = pd.read_csv(filename)
    df['length'] = df.apply(lambda row: len(str(row['TEXT']).split()), axis=1)
    df = df.sort_values(['length'])
    df.to_csv('%s/%s_full_raw.csv' % (args.MIMIC_3_DIR, splt), index=False)
...
counts = Counter()
dfnl = pd.read_csv('%s/notes_labeled_raw.csv' % args.MIMIC_3_DIR)
for row in dfnl.itertuples():
...
for splt in ['train', 'dev', 'test']:
    ...
    with open('%s/notes_labeled_raw.csv' % args.MIMIC_3_DIR, 'r') as f:
        with open('%s/%s_%s_raw.csv' % (args.MIMIC_3_DIR, splt, str(Y)), 'w') as of:
            r = csv.reader(f)
            w = csv.writer(of)
            #header
            w.writerow(next(r))
